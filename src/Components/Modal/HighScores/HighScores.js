...
const HighScores = () => {
    ...
    useEffect(() => {
        const fetchScores = async () => {
            const res = await fetch("https://fir-bee8d.firebaseio.com/antsweeper.json");
            const resData = await res.json();

            const jsonScores = { beginner: [], intermediate: [], expert: [] };
            ...
            for (const level in jsonScores) {
                sortedScores[level] = jsonScores[level].sort((a, b) => a.score > b.score);
            }
